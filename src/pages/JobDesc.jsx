...
const JobDesc = () => {
  ...
  const joblist = job.skills;
  const customerList = fireuser.skills;
  const handleApply = () => {
    ...
    setapplied(true);
    sendListsToServer(joblist, customerList);
  };

  const sendListsToServer = async (joblist, customerList) => {
    fetch("/compare", {
      method: "POST",
      body: JSON.stringify({ list1: joblist, list2: customerList }),
      headers: { "Content-Type": "application/json" },
    })
      .then((response) => response.json())
      .then((JFS) => {
        // setJFS(JFS);
        // const usertoupdate = doc(database, "jfs", user.uid + jobId);
        setDoc(doc(database, "jfs", user.uid + jobId), {
          jfs: JFS.JFS,
        });
        console.log(JFS);
      })
    ...
    <div className="min-h-[630px] mx-[50px]">
      <div class=" rounded-xl overflow-hidden bg-white   justify-between items-center my-1 mx-1 p-3">
        <div class="px-6 py-4 flex items-center">
          <img
            ...
            src={
              job.logo
                ? job.logo
                : "https://img-prod-cms-rt-microsoft-com.akamaized.net/cms/api/am/imageFileData/RE4wB6L?ver=72af"
            }
            alt="Sunset in the mountains"
          />
          <div className="flex flex-col ml-5">
            <div class="font-bold text-3xl mb-3">{job.title}</div>
            <div className="flex font-semibold text-green-600 h-min  mb-2 text-sm bg-green-200 border-2 border-green-500  rounded-xl px-2">
              {job.ename}
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-between">
          ...
          <div>
            {applied || fireuser?.appliedfor?.includes(jobId) ? (
              <button
                // onClick={() => navigate(`/job/${job.job_id}`)}
                className="bg-green-500 text-white font-semibold rounded-xl p-1 border-2 border-green-600 hover:bg-green-700 mr-10 flex items-center gap-2"
              >
                Applied ! <AiOutlineCheckCircle />
              </button>
            ) : (
              <button
                // onClick={() => navigate(`/job/${job.job_id}`)}
                className="bg-blue-500 text-white font-semibold rounded-xl p-1 border-2 border-blue-600 hover:bg-blue-700 mr-10"
                onClick={handleApply}
              >
                Apply Now
              </button>
